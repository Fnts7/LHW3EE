...
import class CInventoryComponent extends CComponent
{
	...
	public function SetupFunds()
	{
		...
		{
			AddMoney( (int)( 1050 * GetFundsModifier() ) );
		}
		else if ( EInventoryFunds_Avg == fundsType )
		{
			AddMoney( (int)( 2200 * GetFundsModifier() ) );
		}
		else if ( EInventoryFunds_Rich == fundsType )
		{
			AddMoney( (int)( 4000 * GetFundsModifier() ) );
		}
		else if ( EInventoryFunds_RichQuickStart == fundsType )
		{
	...
	public function RemoveMoney( amount : int )
	{
		...
		if ( amount > 0 )
		{
			...
			if( thePlayer == GetEntity() && amount > money )
			{
				diff = amount - money;
    ...
    public final function GetItemLevel(item : SItemUniqueId) : int
    {
		...
		var itemName, levelAbility : name;
				...
				switch(itemCategory)
				{
					case 'armor' :
						for (i=0; i<levelDiff; i+=1)
						{
							...
							if( levelAbility != '' && i < 65 )
								AddItemCraftedAbility(item, levelAbility, true);
						}
	...
	// W3EE - Begin
	private function RelevantMappinsInRange() : bool
	{
		var mappinsInRange 	: array<CGameplayEntity>;
		var worldPath		: string;
		var localMapPins	: array< SCommonMapPinInstance >;
		var i 				: int;
		
		localMapPins = theGame.GetCommonMapManager().GetMapPinInstances( theGame.GetWorld().GetDepotPath() );
		if( localMapPins.Size() > 0 )
		{
			for(i=0; i<localMapPins.Size(); i+=1)
			{
				if( localMapPins[i].type == 'MonsterNest' || localMapPins[i].type == 'TreasureHuntMappin' || localMapPins[i].type == 'SpoilsOfWar' || localMapPins[i].type == 'BanditCamp' || localMapPins[i].type == 'BanditCampfire'	|| localMapPins[i].type == 'BossAndTreasure' || localMapPins[i].type == 'Contraband' || localMapPins[i].type == 'ContrabandShip' )
				{
					if( localMapPins[i].radius > 4 && VecDistance( localMapPins[i].position, GetEntity().GetWorldPosition() ) <= localMapPins[i].radius )
					{
						return true;
					}
					else
					if( VecDistance( localMapPins[i].position, GetEntity().GetWorldPosition() ) <= 4 )
					{
						return true;
					}
				}
		...
		FindGameplayEntitiesInRange( mappinsInRange, GetEntity(), 1, 999,'',,,'CR4MapPinEntity' );
		if( mappinsInRange.Size() > 0 )
		{
			for( i = 0; i < mappinsInRange.Size(); i += 1 )
			{
				if ( (W3POI_BanditCampEntity)mappinsInRange[i] || (W3POI_BossAndTreasureEntity)mappinsInRange[i] )
				{
					return true;
				}
			}
		...
		FindGameplayEntitiesInRange( mappinsInRange, GetEntity(), 20, 999,'',,,'CR4MapPinEntity' );
		if ( mappinsInRange.Size() > 0 )
		{
			for( i = 0; i < mappinsInRange.Size(); i += 1 )
			{
				if ( (W3POI_ContrabandEntity)mappinsInRange[i] || (W3POI_ContrabandShipEntity)mappinsInRange[i] || (W3POI_SpoilsOfWarEntity)mappinsInRange[i] )
				{
					return true;
				}
			}
	...
	public function IsEmpty( optional bSkipNoDropNoShow : bool ) : bool
	{
		...
		if( !HasQuestItem() && !theGame.questInventoryComponents.Contains(this) && !((W3treasureHuntContainer)GetEntity()) && !((W3Container)GetEntity()).IsSpecialContainer() && !RelevantMappinsInRange() )
		{
			for(i=itemIds.Size()-1; i>=0; i-=1)
			{
				if( ItemHasTag( itemIds[ i ], 'EmptyContainer' ) )
				{
					empty = true;
				}
				
				if( ItemHasTag( itemIds[ i ], 'ReadableItem' ) )
				{
					return false;
				}
			}
		...
		if(bSkipNoDropNoShow)
		{
			...
			for( i = itemIds.Size() - 1; i >= 0; i -= 1 )
			{
				...
				if( !ItemHasTag( itemIds[ i ],theGame.params.TAG_DONT_SHOW ) && !ItemHasTag( itemIds[ i ], 'NoDrop' ) && !ItemHasTag( itemIds[ i ], 'EmptyContainer' ) )
				{
					return false;
				}
	...
	// W3EE - Begin
	public function IsItemAlchemicalAlcohol( item : SItemUniqueId ) : bool
	{
		return ItemHasTag(item, 'StrongAlcohol');
	}
	
	public function IsItemHerb( item : SItemUniqueId ) : bool
	{
		return ItemHasTag(item, 'primer_herb');
	}
	
	public function IsItemMonsterPart( item : SItemUniqueId ) : bool
	{
		return ItemHasTag(item, 'primer_monstrous');
	}
	
	public function IsItemMineral( item : SItemUniqueId ) : bool
	{
		return ItemHasTag(item, 'primer_mineral');
	}
	...
	public function GetItemScaledPrice( currPrice : float, item : SItemUniqueId, _inv : CInventoryComponent, trade : bool, optional playerSellingItem : bool, optional noDurability : bool ) : int
	{
		var itemCategory 	: name;
		var itemRarity 		: int;
		var itemDurability 	: float;
		var qualityMult    	: float;
		var categoryMult   	: float;
		var sellingMult    	: float;
		var durabilityMult 	: float;
		var areaMult		: float;
		
		itemCategory = _inv.GetItemCategory(item);
		itemRarity = _inv.GetItemQuality(item);
		itemDurability = _inv.GetItemDurabilityRatio(item);
		
		if( itemCategory == 'silversword' || itemCategory == 'steelsword' )
		{
			if( ItemHasTag(item, 'Aerondight') )
				currPrice = 5000.f;
			else
			if( ItemHasTag(item, 'Wooden') || ItemHasTag(item, 'DamagedWeapon') )
				currPrice *= 0.0005f;
			else
			if( ItemHasTag(item, 'SecondaryWeapon') )
			{
				switch( itemRarity )
				{
						case 1: currPrice *= 0.001f; 	break;
						case 2: currPrice *= 0.003f;	break;
						case 3: currPrice *= 0.004f;	break;
						case 4: currPrice *= 0.020f;	break;
						case 5: currPrice *= 0.025f;	break;
				}
			}
			else
			{
				if( itemCategory == 'silversword' )
				{
					switch( itemRarity )
					{
						case 1: currPrice *= 0.005f; 	break;
						case 2: currPrice *= 0.015f; 	break;
						case 3: currPrice *= 0.024f; 	break;
						case 4: currPrice *= 0.500f; 	break;
						case 5: currPrice *= 0.120f;	break;
					}
				}
				else
				{
					switch( itemRarity )
					{
						case 1: currPrice *= 0.002f; 	break;
						case 2: currPrice *= 0.005f;	break;
						case 3: currPrice *= 0.009f;	break;
						case 4: currPrice *= 0.190f;	break;
						case 5: currPrice *= 0.045f;	break;
					}
				}
			}
		}
		...
		if( itemCategory == 'armor' )
		{
			switch( itemRarity )
			{
				case 1: currPrice *= 0.14f; 	break;
				case 2: currPrice *= 0.20f; 	break;
				case 3: currPrice *= 0.24f; 	break;
				case 4: currPrice *= 0.27f; 	break;
				case 5: currPrice *= 0.32f; 	break;
			}
		}
		else
		if( itemCategory == 'boots' )
		{
			switch( itemRarity )
			{
				case 1: currPrice *= 0.20f; 	break;
				case 2: currPrice *= 0.25f; 	break;
				case 3: currPrice *= 0.30f; 	break;
				case 4: currPrice *= 0.31f; 	break;
				case 5: currPrice *= 0.33f; 	break;
			}
		}
		else
		if( itemCategory == 'gloves' )
		{
			switch( itemRarity )
			{
				case 1: currPrice *= 0.25f; 	break;
				case 2: currPrice *= 0.30f; 	break;
				case 3: currPrice *= 0.35f; 	break;
				case 4: currPrice *= 0.38f; 	break;
				case 5: currPrice *= 0.42f; 	break;
			}
		}
		else
		if( itemCategory == 'pants' )
		{
			...
			{
				case 1: currPrice *= 0.25f; 	break;
				case 2: currPrice *= 0.35f; 	break;
				case 3: currPrice *= 0.45f; 	break;
				case 4: currPrice *= 0.55f; 	break;
				case 5: currPrice *= 0.65f; 	break;
			}
		}
		else
		if( IsItemBolt(item) )
		...
		if( IsItemCraftingIngredient(item) )
		{
			switch( itemRarity )
			{
				case 1: currPrice *= 0.30f; 	break;
				case 2: currPrice *= 0.30f; 	break;
				case 3: currPrice *= 0.30f; 	break;
				case 4: currPrice *= 0.30f; 	break;
				case 5: currPrice *= 0.40f; 	break;
			}
		}
		
		categoryMult = LFEGetCategoryMult(itemCategory);
		qualityMult = LFEGetQualityMult(itemRarity);
		areaMult = GetAreaPriceMult(item, itemRarity);
		
		if( !noDurability )
			durabilityMult = LFEGetDurabilityMult(itemDurability);
		else
			durabilityMult = 1.f + (1 - _inv.GetItemDurability(item) / _inv.GetItemMaxDurability(item));
		
		if( trade )
		{
			sellingMult = LFEGetSellingMult(playerSellingItem);
			if( !playerSellingItem )
				return Max(1, CeilF(currPrice * qualityMult * categoryMult * sellingMult * durabilityMult * areaMult));
			else
				return FloorF(currPrice * qualityMult * categoryMult * sellingMult * durabilityMult * areaMult);
		}
		else
		{
			sellingMult = 1;
	...
	public final function AddAnItem(item : name, optional quantity : int, optional dontInformGui : bool, optional dontMarkAsNew : bool, optional showAsRewardInUIHax : bool) : array<SItemUniqueId>
	{
		...
		if( theGame.GetDefinitionsManager().IsItemSingletonItem(item) && GetEntity() == thePlayer)			
		{
			...
			// W3EE - Begin
			if( IsItemOil(arr[0]) )
				Alchemy().AddOilTagsOnCook(arr[0], this);
			// W3EE - End
	...
	// W3EE - Begin
	public function AddAndEquipItem( item : name, slot : EEquipmentSlots, optional quantity : int )
	{
		var items : array <SItemUniqueId>;
	...
	public function ReduceItemDurability(itemId : SItemUniqueId, optional forced : bool) : bool
	{
		...
		if(IsItemWeapon(itemId))
		{
			...
			if( thePlayer.HasAbility('Runeword 8 _Stats', true) || thePlayer.HasAbility('Runeword 12 _Stats', true) || thePlayer.HasAbility('Runeword 11 _Stats', true) )
			{
				value *= 0;
				chance = 0;
			}
		...
		else if(IsItemAnyArmor(itemId))
		{
			...
			{
				case EAT_Heavy: chance = 45; break;
				case EAT_Medium: chance = 55; break;
				case EAT_Light: chance = 70; break;
			}
			//W3EE - End
	...
	public final function GetParamsForRunewordTooltip(runewordName : name, out i : array<int>, out f : array<float>, out s : array<string>)
	{
		...
		switch(runewordName)
		{
			...
			// W3EE - Begin
			case 'Glyphword 8' :
				theGame.GetDefinitionsManager().GetAbilityAttributeValue('Glyphword 8 _Stats', 'glyphword8_chance', min, max);
				i.PushBack( RoundMath( CalculateAttributeValue(min) * 100) );			
				theGame.GetDefinitionsManager().GetAbilityAttributeValue('Glyphword 8 _Stats', 'glyphword8_range', min, max);
				i.PushBack( RoundMath( CalculateAttributeValue(min) * 0.1f) );
				theGame.GetDefinitionsManager().GetAbilityAttributeValue('Glyphword 8 _Stats', 'spell_power_igni', min, max);
			...
			// W3EE - Begin
			case 'Glyphword 13' :
				theGame.GetDefinitionsManager().GetAbilityAttributeValue('Glyphword 13 _Stats', 'increas_duration', min, max);
				val = CalculateAttributeValue(min);
				s.PushBack( NoTrailZeros(val) );
				theGame.GetDefinitionsManager().GetAbilityAttributeValue('Glyphword 13 _Stats', 'spell_power_axii', min, max);
				i.PushBack( RoundMath( min.valueMultiplicative * 100) );
				break;
			// W3EE - End
			case 'Glyphword 14' :
	...
	public final function GetPotionAttributesForTooltip(potionId : SItemUniqueId, out tips : array<SAttributeTooltip>):void
	{
		...
		// W3EE - Begin
		else if(buffType == EET_Swallow)
		{
			attrs.Remove('vitalityCombatRegen');
		}
		// W3EE - End
		...
		else if(buffType == EET_Thunderbolt)
		{
			...
			attrs.Remove('critical_frenzy_chance');
			attrs.Remove('critical_frenzy_duration');
			attrs.Remove('critical_frenzy_slowdown');
			// W3EE - End
		}
									...
									out itemStats : array<SAttributeTooltip>, out localizedFluff : string)
	{
		...
		price = GetItemScaledPrice(price, itemId, this, true, false);
		// W3EE - End
	...
	public function SingletonItemGetMaxAmmo(itemID : SItemUniqueId) : int
	{
		...
		if( !ItemHasTag( itemID, 'NoAdditionalAmmo' ) )
		{
			if(GetEntity() == GetWitcherPlayer() && ammo > 0)
			{
				...
				if(thePlayer.HasBuff(EET_AlchemyTable) && !IsItemMutagenPotion(itemID))
				{
					ammo += 1;
				}
		...
		if ( IsItemEncumbranceItem( item ) )
		{
			...
			else if ( IsItemQuest( item ) )
			{
				if( Options().WeightQuest() )
				{
					...
					{
						return weight * GetItemQuantity( item );
					}
					else
					{
						return 0.05 * GetItemQuantity( item );
					}
				}
				else return 0;
			}	
			...
			else if ( IsItemSingletonItem( item ) && IsItemAlchemyItem( item ) )
			{
				...
				{
					return 0.25 * GetItemQuantity( item ) * GetItemModifierInt( item, 'ammo_current' );
				}
				else if ( IsItemBomb( item ) )
				{
					return 0.5 * GetItemQuantity( item ) * GetItemModifierInt( item, 'ammo_current' );
				}
			}
			else 
			{
	...
	event OnItemAdded(data : SItemChangedData)
	{
		...
		if( !ItemHasTag(itemId, 'SilverOil') && ItemHasTag(itemId, 'SteelOil') )
			AddItemTag(itemId, 'SilverOil');
		if( !ItemHasTag(itemId, 'SteelOil') && ItemHasTag(itemId, 'SilverOil') )
			AddItemTag(itemId, 'SteelOil');
		// W3EE - End
