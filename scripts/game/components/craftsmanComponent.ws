	...
	public function CalculateCostOfCrafting( craftedItemName : name ) : int
	{
		...
		if ( owner )
		{
			if ( owner.invComp )
			{
				...
				{
					// W3EE - Begin
					craftingCost = owner.invComp.GetItemPrice(items[0]) / 2;
					if( owner.invComp.GetItemQuality(items[0]) <= 4 )
						craftingCost = RoundMath(owner.invComp.GetItemScaledPrice(craftingCost, items[0], owner.invComp, true, false) * 0.9f);
					else
						craftingCost = RoundMath(owner.invComp.GetItemScaledPrice(craftingCost, items[0], owner.invComp, true, false) * 0.6f);
					if( owner.invComp.IsItemBolt(items[0]) )
						craftingCost *= 20;
