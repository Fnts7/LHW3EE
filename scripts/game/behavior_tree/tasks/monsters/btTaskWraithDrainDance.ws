...
class CBTTaskWraithDrainDance extends CBTTaskPlayAnimationEventDecorator
{
	...
	latent function Main() : EBTNodeStatus
	{
		...
		// W3EE - Begin
		summonedEntityComponent = (W3SummonedEntityComponent) l_npc.GetComponentByClassName('W3SummonedEntityComponent');
		l_targetNode			= l_npc.GetComponent("DrainEnergyTarget");
		if( !l_targetNode )
		{
			l_targetNode = l_npc;
		}
		l_sourceNode			= l_target.GetComponent("torso3effect");
		if( !l_sourceNode )
		{
			l_sourceNode = l_target;
		}
		...
		{			
			...
			if( l_dist < 4 && l_target && (CR4Player)l_target )
			{
				if( !l_target.HasBuff( EET_VitalityDrain ) )
				{
					AddDrainBuff(l_target);
				}				
	...
	private function AddDrainBuff( l_actor : CActor )
	{
		var i			: int;
