			...
			if( l_actors.Size() > 0 )
			{
				...
				if( IsNameValid( weaponSlot ) )
				{
					...
					*/
					...
					if ( theGame.GetDifficultyMode() == EDM_Easy )     damageValue = 700  /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 2 )*/; else
					if ( theGame.GetDifficultyMode() == EDM_Hard )     damageValue = 1250 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 4 )*/; else
					if ( theGame.GetDifficultyMode() == EDM_Hardcore ) damageValue = 2000 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 5 )*/; else
																	   damageValue = 950 /*+ ( ( l_damageAttr.valueBase * l_damageAttr.valueMultiplicative + l_damageAttr.valueAdditive ) * 3 )*/;
					// W3EE - End
				}
				...
				for	( i = 0; i < l_actors.Size(); i += 1 )
				{
					...
					// W3EE - Begin
					l_damageAction.AddDamage(  theGame.params.DAMAGE_NAME_DIRECT, damageValue );
