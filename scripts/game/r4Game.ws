...
import class CR4Game extends CCommonGame
{
	...
	public var Crimes : W3EECrimeSystemHandler;
	...
	private function SetTimescaleSources()
	{
		...
		timescaleSources[ ETS_PotionBlizzard ].sourcePriority = 25;
		...
		timescaleSources[ ETS_ThrowingAim ].sourcePriority = 20;
		...
		timescaleSources[ ETS_RadialMenu ].sourcePriority = 25;
		...
		timescaleSources[ ETS_SkillFrenzy ].sourcePriority = 25;
		...
		timescaleSources[ ETS_Yrden ].sourcePriority = 25;
		...
		timescaleSources[ ETS_CockatriceMutagen ].sourcePriority = 25;
	...
	event OnGameStarting(restored : bool )
	{
		...
		// W3EE - Begin
		questInventoryComponents.Clear();
		questInventoryComponents.Resize(0);
		...
		Crimes = new W3EECrimeSystemHandler in this;
	...
	event OnGiveReward( target : CEntity, rewardName : name, rewrd : SReward )
	{
		...
		if ( gameplayEntity )
		{
			...
			if ( inv )
			{
				...
				{
					// W3EE - Begin
					inv.AddMoney( (int)( rewrd.gold * goldMultiplier * Options().QuestGoldMult()) );
					
					thePlayer.RemoveRewardMultiplier(rewardName);		
					if( target == thePlayer )
					{
						moneyWon = (int)( rewrd.gold * goldMultiplier * Options().QuestGoldMult() );
...
function Crimes() : W3EECrimeSystemHandler
{
	return theGame.Crimes;
}
