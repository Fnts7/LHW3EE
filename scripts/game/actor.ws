...
import abstract class CActor extends CGameplayEntity
{
	...
	private function CalculateFinalAnimationSpeedMultiplier( optional forceValue : bool ) : float
	{
		...
		if(size > 0)
		{
			...
			if( !forceValue )
			{
				if ( FinalMult > 2.2 && Options().SpeedCap() )
					return MinF(FinalMult, 2.2f);
				else
				if ( FinalMult < 0.55 && Options().SpeedCapLow() )
					return MaxF(FinalMult, 0.55f);
				else return FinalMult;
			}
			else return FinalMult;
		}
		return 1;
		...
		*/
		...
		if(playerAttacker)
		{
			if (thePlayer.HasBuff(EET_Mutagen07))
			{
				...
				lifeLeech = ((W3Mutagen07_Effect)mutagen).GetLeechPercent();
				// W3EE - End
				if (UsesVitality())
	...
	// W3EE - Begin
	private var burnCounter : int;	default burnCounter = 0;
	public function GetBurnCounter() : int
	{
		return burnCounter;
	}
	
	public function IncBurnCounter()
	{
		burnCounter += 1;
	}
	// W3EE - End
	...
	public function GetNumMutagensActive() : int
	{
		if( effectManager && effectManager.IsReady() )
			return effectManager.GetNumMutagensActive();
	...
	protected function PrepareAttackAction( hitTarget : CGameplayEntity, animData : CPreAttackEventData, weaponId : SItemUniqueId, parried : bool, countered : bool, parriedBy : array<CActor>, attackAnimationName : name, hitTime : float, weaponEntity : CItemEntity, out attackAction : W3Action_Attack) : bool
	{
		...
		if(phantomStrike)
		{
			...
			if( attackAction.attacker == thePlayer )
			{
				...
				phantomDmg.AddDamage(theGame.params.DAMAGE_NAME_DIRECT, (phantomDmgInfo.valueBase * (1.f + phantomDmgInfo.valueMultiplicative) + phantomDmgInfo.valueAdditive));
