...
statemachine import class CNewNPC extends CActor
{
	 ...
	 event OnProcessActionPost(action : W3DamageAction)
	{
		...
		// W3EE - Begin
		if(HasBuff(EET_AxiiGuardMe) && (thePlayer.HasAbility('Glyphword 14 _Stats', true) || thePlayer.HasAbility('Glyphword 18 _Stats', true) || thePlayer.HasAbility('Glyphword 13 _Stats', true)) && action.DealtDamage())
		// W3EE - End
		{
	...
	public function AddDamageImmunity( damageName : name )
	{
		...
		switch(damageName)
		{		
			case theGame.params.DAMAGE_NAME_FROST 		: effects.PushBack(EET_Frozen);	break;
	...
	public function GetTotalArmorReduction() : float
	{
		return AbsF(armorReducedBy);
	}
	
	public function ModifyArmorValue( by : float )
	{
	...
	function GetExperienceDifferenceLevelName( out strLevel : string ) : string
	{
		...
		var Options : W3EEOptionHandler;
		var Fixes : W3EEExperienceHandler;
		
		Fixes = new W3EEExperienceHandler in this;
...
/*		if ( !Options().Scale() )
	...
	{		
		...
		Combat().CombustionSkill(damageAction);
		...
		if(damageAction.attacker == thePlayer && ((W3PlayerWitcher)thePlayer) && thePlayer.CanUseSkill(S_Alchemy_s17) && thePlayer.GetStat(BCS_Toxicity, false) > witcher.GetToxicityDamageThreshold() )
		{
	...
	private function StartRegenTimer()
	{
		isRegenActive = false;
		AddTimer('SetRegenActive', npcStats.regenDelay, false,,,,true);
	}
	// W3EE - End
	...
	event OnTakeDamage( action : W3DamageAction )
	{
		...
		// W3EE - Begin
		if( npcStats.healthRegenFactor > 0 && npcStats.regenDelay > 0 && action.DealsAnyDamage() )
			StartRegenTimer();
		// W3EE - End
		...
		if(action.IsActionMelee() && action.DealsAnyDamage())
		{
			...
			if(witcher && witcher.HasBuff(EET_Mutagen10))
			{
				...
				if (addAbility)
				{
					thePlayer.AddAbilityMultiple(abilityName, i);
				}
			}
