...
class CPlayerInput 
{
	...
	event OnAccessHorseStash( action : SInputAction )
	{
		if( IsReleased(action) )
		{
			...
			{
				theGame.GetGuiManager().ShowNotification("Horse is out of range.");
				return false;
			}
	...
	public function GetWasStashCall() : bool
	{
		return wasStashCall;
	}
	
	public function SetWasStashCall()
	{
		wasStashCall = false;
	}
	...
	{
		if( IsPressed(action) && !thePlayer.IsInCombat() && !thePlayer.GetIsSprinting() && !thePlayer.modifyPlayerSpeed )
		{
			// W3EE - Begin
	...
	event OnCbtAttackLight( action : SInputAction )
	{
		...
		{
			// W3EE - Begin
			if( thePlayer.IsPlayerAttacking() )
				thePlayer.SetPlayerSpamming(true);
