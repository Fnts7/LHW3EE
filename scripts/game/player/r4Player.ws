...
{ 
	...
	private				var isShootingFriendly			: bool;
		...
		// W3EE- Begin
		default findMoveTargetDistMax = 30.f;
		// W3EE - End
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		// W3EE - Begin
		AddTimer('RefreshDamSett', 2, false);
	...
	public function ApplyOil( oilId : SItemUniqueId, usedOnItem : SItemUniqueId ) : bool
	{
		...
		*/
		...
		{
			// W3EE - Begin
			ammoBonus = 0.3f; //CalculateAttributeValue(GetSkillAttributeValue(S_Alchemy_s06, 'ammo_bonus', false, false));
			// W3EE - End
	...
	protected function PerformCounterCheck(parryInfo: SParryInfo) : bool
	{
		...
		if( !HasStaminaToUseAction(ESAT_Counterattack, , mult) )
			return false;
		//W3EE - End
	...
	function SetOrientationTarget( target : EOrientationTarget )
	{
			...
			if ( !Options().LockOnMode() && !Options().LockOn() )
			{
		...
		if ( customOrientationInfoStack.Size() > 0 )
		{
			for( i = customOrientationInfoStack.Size()-1; i>=0; i-=1 )
			{
				if ( customOrientationInfoStack[i].sourceName == sourceName )
				{
					if ( !Options().LockOnMode() && !Options().LockOn() )
					{
						customOrientationInfoStack[i].customHeading = VecHeading( theCamera.GetCameraDirection() ) - oTCameraOffset;
					}
					else
					{
	...
	{	
		// W3EE - Begin
		if ( ( actionType == 0 && /*theInput.GetActionValue( 'CastSignHold' ) == 0.f*/ !GetWitcherPlayer().GetIsAlternateCast() ) 
		// W3EE - End
	...
	public function CanSprint( speed : float ) : bool
	{
		...
		{
			// W3EE - Begin
			if ( !Options().SprintInWitcherSense() )
				return false;
			// W3EE - End
		}
	...
	protected function FindTargetSimple()
	{
		...
		for( i = targets.Size() - 1; i >= 0; i -= 1 )
		{
			...
			if ( !IsThreat( targets[i], false ) && !targets[i].HasTag('animal') )
			{
		...
		if (targets.Size() > 0)
		{
			for( i = 0; i < targets.Size(); i += 1 )
			{
				inverseDist = (((targetingInfo.coneDist + 2.0f) - Distance2DBetweenCapsules( this, targets[i] )) / (targetingInfo.coneDist/2.0f + 1.0f));
				...
				rank = ((180.0f - cameraAngleDiff) / 180.0f) * inverseDist * cameraWeight;
				rank += ((180.0f - facingAngleDiff) / 180.0f) * inverseDist * facingWeight;
				
				if ( lastAxisInputIsMovement )
					rank += ((180.0f - movementAngleDiff) / 180.0f) * inverseDist * movementWeight;
			...
			if ( newTarget && ( (newTarget == currentTarget) || (curTargetRank < 0) || ((curTargetRank > 0) && (curTargetRank * targetHysteresis < bestRank)) ) )
			{
				targetingOut.result = true;
				targetingOut.target = newTarget;
	...
	protected function FindTarget( optional actionCheck : bool, optional action : EBufferActionType, optional actionInput : bool ) : CActor
	{
		...
		// W3EE - Begin	
		if ( !Options().LockOnModeSF() && Options().LockOn() )
		{
			FindTargetSimple();
		}
		else
	...
	function PerformParryCheck( parryInfo : SParryInfo ) : bool
	{
		...
		{	
			...
			if( GetSkillLevel(S_Sword_s10) < 3 && parryDir == PPD_Back )
				return false;
			...
			if ( parryInfo.targetToAttackerDist > 3.f && !bLAxisReleased && !thePlayer.IsCiri() )
			{
				...
				{
					if( !CanUseSkill(S_Sword_s10) )
						Combat().StaminaLoss(ESAT_Parry);
	...
	protected function DoAttack(animData : CPreAttackEventData, weaponId : SItemUniqueId, parried : bool, countered : bool, parriedBy : array<CActor>, attackAnimationName : name, hitTime : float)
	{
		...
		*/
		...
		if( IsInCombatAction_SpecialAttack() || IsSpecialAttack(animData.attackName) || IsCounterAttack(animData.attackName) )
			return;
		...
		switch(attackDistance)
		{
			...
			case ADIST_Small:	
				if( IsHeavyAttack(animData.attackName) )
					Combat().StaminaLoss(ESAT_HeavyAttack);
				else
					Combat().StaminaLoss(ESAT_LightAttack);
			break;
	...
	public function HasStaminaToUseAction(action : EStaminaActionType, optional abilityName : name, optional dt :float, optional multiplier : float) : bool
	{
		...
		cost = Combat().GetActionStaminaCost(action, temp, multiplier, dt, abilityName);
	...
	// W3EE - Begin
	public function DrainFocus(amount : float, optional dt : float, optional isUndyingCall : bool )
	{
		if(abilityManager && abilityManager.IsInitialized() && IsAlive())
			abilityManager.DrainFocus(amount, dt, isUndyingCall);
	}
	// W3EE - End
	...
	public function IsLightAttack(attackName : name) : bool
	{
		...
		// W3EE - Begin
		if ( attackName == 'attack_light_special' || StrContains(NameToString(attackName), "counter") )
			return true;
		// W3EE - End
	...
	{	
		// W3EE - Begin
		var combatActionType : float = GetBehaviorVariable('combatActionType');
		
		if( combatActionType == 0 )
	...
	// W3EE - Begin
	private var isPlayerAttacking : bool;	default isPlayerAttacking = false;
	private var wasPlayerSpamming : bool;	default wasPlayerSpamming = false;
	private var isPlayerSpamming : bool;	default isPlayerSpamming = false;
	public function SetPlayerSpamming( flag : bool )
	{
		isPlayerSpamming = flag;
	}
	
	public function SetWasPlayerSpamming( flag : bool )
	{
		wasPlayerSpamming = flag;
	}
	
	public function SetPlayerAttacking( flag : bool )
	{
		isPlayerAttacking = flag;
	}
	
	public function IsPlayerAttacking() : bool
	{
		return isPlayerAttacking;
	}
	
	public function WasPlayerSpamming() : bool
	{
		return wasPlayerSpamming;
	}
	
	public function IsPlayerSpamming() : bool
	{
		return isPlayerSpamming;
	}
	
	timer function ResetPlayerSpamming( dt : float, id : int )
	{
		wasPlayerSpamming = false;
	}
}
