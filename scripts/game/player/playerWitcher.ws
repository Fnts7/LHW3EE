...
{		
	...
	timer function RefreshVigor( dt : float, id : int )
	{
		...
		if( FactsQuerySum("HoodsHairHidden") )
			Equipment().ToggleHairItem(false, this, true);
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		AddTimer('RefreshVigor', 1.5, false);
		
		AddTimer('InitSkills', 2.5, false);
		
		AddTimer('W3EEBestiaryInitialize', 2.5, false);
	...
	event OnTakeDamage( action : W3DamageAction)
	{
		...
		if( action.GetHitReactionType() != EHRT_None && !action.IsDoTDamage() )
		{
			ResetCustomAnimationSpeedMult();
			Combat().RemovePlayerSpeedMult();
		}
		GetConsumableState().OnTakeDamage(action);
		// W3EE - End
		...
		if(action.DealsAnyDamage() && !((W3Effect_Toxicity)action.causer) )
		{
			...
			{
				mutagen03 = (W3Mutagen03_Effect)GetBuff(EET_Mutagen03);
				action.MultiplyAllDamageBy(mutagen03.GetAmplification());
	...
	event OnProcessActionPost(action : W3DamageAction)
	{
		...
		if( attackAction && attackAction.IsActionMelee() && !IsUsingHorse() && attackAction.DealtDamage() && IsSetBonusActive( EISB_Lynx_1 ) && !attackAction.IsCountered() && ( inv.IsItemSteelSwordUsableByPlayer( attackAction.GetWeaponId() ) || inv.IsItemSilverSwordUsableByPlayer( attackAction.GetWeaponId() ) ) )
		{
	...
	event OnCombatStart()
	{
		...
		if( IsSetBonusActive(EISB_Dimeritium1) && !HasBuff(EET_DimeritiumCharge) )
			AddEffectDefault(EET_DimeritiumCharge, this, "DimeritiumSetBonus");
		...
		{
			focus = GetStat(BCS_Stamina);
			if(focus < (GetStatMax(BCS_Stamina) / 5) * GetSkillLevel(S_Sword_s20))
			{
				GainStat(BCS_Stamina, (GetStatMax(BCS_Stamina) / 5) * GetSkillLevel(S_Sword_s20) - focus);
			}
		}
		// W3EE - End
	...
	public function ConsumeItem( itemId : SItemUniqueId ) : bool
	{
		...
		if (itemName == 'q111_imlerith_acorn' ) 
		{
			...
			Experience().ModTotalPathPoints(ESSP_Sword_Utility, 1);
	...
	public function GetTotalArmor() : SAbilityAttributeValue
	{
		...
		if(GetItemEquippedOnSlot(EES_Armor, armorItem))
		{
			...
			tempArmor = inv.GetItemArmorTotal(armorItem);
			tempArmor.valueBase *= MaxF(0, 1.f + ((tempArmor.valueBase - 131) / 660));
			armor += tempArmor;			
		}
		...
		if(GetItemEquippedOnSlot(EES_Pants, armorItem))
		{
			...
			tempArmor = inv.GetItemArmorTotal(armorItem);
			tempArmor.valueBase *= MaxF(0, 1.f + ((tempArmor.valueBase - 40) / 171));
			armor += tempArmor;			
		}
		...
		if(GetItemEquippedOnSlot(EES_Gloves, armorItem))
		{
			...
			tempArmor = inv.GetItemArmorTotal(armorItem);
			tempArmor.valueBase *= MaxF(0, 1.f + ((tempArmor.valueBase - 38) / 166));
			armor += tempArmor;			
		}
			
		if( HasAbility('Glyphword 11 _Stats', true) )
		{
			if (GetItemEquippedOnSlot(EES_Armor, armorItem))
			{
	...
	{			
		...
		if( slot == EES_Quickslot1 || slot == EES_Quickslot2 )
		{
			if( inv.ItemHasTag(item, 'Hood') )
			{
				...
				{
					Equipment().ToggleHairItem(false, this);
					FactsAdd("HoodsHairHidden");
				}
			}
		}
		// W3EE - End
	...
	public function UnequipItemFromSlot(slot : EEquipmentSlots, optional reequipped : bool) : bool
	{
		...
		if( slot == EES_Quickslot1 || slot == EES_Quickslot2 )
		{
			if( inv.ItemHasTag(item, 'Hood') )
			{
				Equipment().ToggleHairItem(true, this);
				FactsRemove("HoodsHairHidden");
			}
		}
	...
	public function GetMaxRunEncumbrance(out usesHorseBonus : bool) : float
	{
		...
		value += Options().BaseCWGeralt() + CalculateAttributeValue( GetAttributeValue('encumbrance') ) - 60;
		// W3EE - End
	...
	event OnProcessCastingOrientation( isContinueCasting : bool )
	{
		...
		if ( GetDisplayTarget() && this.IsDisplayTargetTargetable() )
		{
			...
			// W3EE - Begin
			if ( /*theInput.GetActionValue( 'CastSignHold' ) > 0*/ GetWitcherPlayer().GetIsAlternateCast() || this.IsCurrentSignChanneled() )
			// W3EE - End
			{
				if ( IsPCModeEnabled() )
				{
		...
		{
			// W3EE - Begin
			if ( GetPlayerCombatStance() == PCS_AlertNear && GetWitcherPlayer().GetIsAlternateCast() /*theInput.GetActionValue( 'CastSignHold' ) > 0*/ )
			// W3EE - End
			{
	...
	{
		if( equippedSign == ST_Aard || equippedSign == ST_Quen )
		{
	...
	public function CastDesiredSign( signType : ESignType, freeCast : bool, alternateCast : bool, spawnPos : Vector, rotation : EulerAngles )
	{
		var signEntity : W3SignEntity;
	...
	public final function CalculatePotionDuration(item : SItemUniqueId, isMutagenPotion : bool, optional itemName : name) : float
	{
		...
		if( inv.GetItemName(item) != 'White Raffards Decoction 3' )
		{
			...
			if( Options().GetPotionDurationMult() && !isMutagenPotion )
	...
	{	
		...
		if (!inv.GetItemModifierInt(item, 'ammo_current') )
		{
			if( GetItemSlot(item) != EES_InvalidSlot )
				UnequipItem(item);
	...
	public function QuenImpulse( isAlternate : bool, signEntity : W3QuenEntity, source : string, optional forceSkillLevel : int )
	{
		...
		ents = GetActorsInRange(this, 3, 1000, , true);
		for(i=0; i<ents.Size(); i+=1)
		{
			if( (W3PlayerWitcher)ents[i] )
				continue;
			...
			if(level == 3)
			{
				...
				{
					impulsePower = 0.35f * sp.valueMultiplicative;
					if( RandF() <= impulsePower )
					{
						if( IsSetBonusActive( EISB_Bear_2 ) )
		...
		GCameraShake(0.2f * sp.valueMultiplicative);
		// W3EE - End
	...
	public function OnSignCastPerformed(signType : ESignType, isAlternate : bool)
	{
		...
		if(HasAbility('Runeword 1 _Stats', true) && !infusionCooldown /*&& GetStat(BCS_Focus) >= 1.0f*/)
		{
			// DrainFocus(1.0f);
			// W3EE - End
	...
	// W3EE - Begin
	public function GetTotalSpellPower() : SAbilityAttributeValue
	{
	...
	{
		return animManager.IsUsingConsumable();
	}
	...
	{
		return animManager.GetConsumableState();
	}
	...
	public function IsSetBonusActive( bonus : EItemSetBonus ) : bool
	{
		switch(bonus)
		{
			...
			case EISB_Bear_1:			return amountOfSetPiecesEquipped[ EIST_Bear ] +  Max(0, amountOfSetPiecesEquipped[ EIST_MinorBear ] - (theGame.params.ITEMS_REQUIRED_FOR_MAJOR_SET_BONUS - theGame.params.ITEMS_REQUIRED_FOR_MINOR_SET_BONUS)) >= theGame.params.ITEMS_REQUIRED_FOR_MINOR_SET_BONUS;
			...
			case EISB_Gothic2:			return amountOfSetPiecesEquipped[ EIST_Gothic ] + IsHelmetEquipped(EIST_Gothic) >= theGame.params.ITEMS_REQUIRED_FOR_MAJOR_SET_BONUS;
			case EISB_Dimeritium1:		return amountOfSetPiecesEquipped[ EIST_Dimeritium ] + IsHelmetEquipped(EIST_Dimeritium) >= theGame.params.ITEMS_REQUIRED_FOR_MINOR_SET_BONUS;
	...
	private var currentHelmet : EItemSetType;
	...
	public function UpdateAllSetBonuses( increment : bool )
	{
		...
		for(i=1; i<=EnumGetMax('EItemSetType')-8; i+=1)
		{
			...
			if( !increment && !IsSetBonusActive( EISB_Wolf_1 ) )
			{
	...
	public function ItemSetTypeToItemSetBonus( setType : EItemSetType, nr : int ) : EItemSetBonus
	{
		...
		if( nr == 1 )
		{
			switch( setType )
			{
				...
				case EIST_Wolf:
				case EIST_MinorWolf: 		setBonus = EISB_Wolf_2;		break;
				case EIST_RedWolf:
	...
	public function GetSetBonusTooltipDescription( bonus : EItemSetBonus ) : string
	{
		...
		switch( bonus )
		{
			case EISB_Lynx_1:			tempString = "W3EE_Cat_1"; break;
			...
			case EISB_Gryphon_2:		tempString = "W3EE_Griffin_1"; break;
			case EISB_Bear_1:			tempString = "W3EE_Bear_1"; break;
			...
			case EISB_Wolf_2:			tempString = "W3EE_Wolf_2_Ability"; break;
	...
	public timer function FinishTarget( dt : float, id : int )
	{
		...
		if( cachedAct && cachedAct.IsAlive() )
		{
			...
			damage = new W3DamageAction in theGame.damageMgr;
			damage.Initialize(this, cachedAct, NULL, "FinisherDamage", EHRT_Light, CPS_AttackPower, true, false, false, false);
			damage.SetHitAnimationPlayType(EAHA_ForceNo);
			damage.SetCannotReturnDamage(true);
			damage.SetCanPlayHitParticle(false);
			damage.AddDamage(theGame.params.DAMAGE_NAME_DIRECT, 99999999);
			...
			if( cachedAct.HasTag('ForceFinisher') )
				cachedAct.AddAbility('ForceFinisher', false);
	...
	public function GetOilCritChanceBonus( victimMonsterCategory : EMonsterCategory ) : SAbilityAttributeValue
	{
		...
		if( oils.Size() > 0 )
		{
			...
			for(i=0; i<oils.Size(); i+=1)
			{
				...
				{
					critChance = inv.GetItemAttributeValue(weaponId, attributeName) * (1.f - PowF(1.f - oils[i].GetAmmoPercentage(), 2));
				}
			}
		}
	...
	public function GetOilCritDamageBonus( victimMonsterCategory : EMonsterCategory ) : SAbilityAttributeValue
	{
		...
		if( oils.Size() > 0 )
		{
			...
			for(i=0; i<oils.Size(); i+=1)
			{
				...
				if( oils[i].GetAmmoCurrentCount() > 0 && theGame.GetDefinitionsManager().ItemHasAttribute(appliedOilName, true, attributeName) )
				{
					critDamage = inv.GetItemAttributeValue(weaponId, attributeName) * (1.f - PowF(1.f - oils[i].GetAmmoPercentage(), 2));
				}
			}
		}
	...
	public function GetOilResistIgnore( victimMonsterCategory : EMonsterCategory ) : float
	{
		...
		if( oils.Size() > 0 )
		{
			...
			for(i=0; i<oils.Size(); i+=1)
			{
				...
				if( oils[i].GetAmmoCurrentCount() > 0 && theGame.GetDefinitionsManager().ItemHasAttribute(appliedOilName, true, attributeName) )
				{
					resistIgnore = inv.GetItemAttributeValue(weaponId, attributeName) * (1.f - PowF(1.f - oils[i].GetAmmoPercentage(), 2));
				}
			}
		}
	...
	public function GetPlayerArmorPiercingValue( isMelee : bool, isRanged : bool, attackName : name ) : float
	{
