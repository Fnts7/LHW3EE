...
class W3AlchemyManager
{
	...
	public function CanCookRecipe(recipeName : name, optional ignorePlayerState:bool, optional isAlchemist : bool ) : EAlchemyExceptions
	{
		...
		var itemName : name;
		var adjusted_ingredient_count : array<int>;
		...
		adjusted_ingredient_count.Resize(recipe.requiredIngredients.Size() );
		for (i = recipe.requiredIngredients.Size() - 1; i >= 0; i-= 1)
		{
			for (j = i; j >= 0; j -= 1)
			{
				if (recipe.requiredIngredients[i].itemName == recipe.requiredIngredients[j].itemName)
					adjusted_ingredient_count[i] += recipe.requiredIngredients[j].quantity;				
			}			
		}	
		...
		for(i=0; i<recipe.requiredIngredients.Size(); i+=1)
		{
			...
			if (availableIngredients < recipe.requiredIngredients[i].quantity || availableIngredients < adjusted_ingredient_count[i])
				return EAE_NotEnoughIngredients;		
		}
		return EAE_NoException;
	}
	
	public function CookItem(recipe : SAlchemyRecipe, quantity : int, optional nigredo, rubedo, albedo : array<name>)
	{
		var i : int;
		...
		var itemToCook, secondarySubstance : name;
		...
		secondarySubstance = Alchemy().GetSecondarySubstance(recipe.requiredIngredients, nigredo, rubedo, albedo);
		if( secondarySubstance != '' && !isDistilling )
			itemToCook = Alchemy().GetPotionNameFromSubstance(recipe.cookedItemName, secondarySubstance);
		else
			itemToCook = recipe.cookedItemName;
		...
		if( dm.IsItemSingletonItem(itemToCook) )
		{
			...
			if( items.Size() == 1 && witcher.inv.ItemHasTag(items[0], 'NoShow') )
				witcher.inv.RemoveItemTag(items[i], 'NoShow');
			
			ids = witcher.inv.AddAnItem(itemToCook, quantity);
		}		
		else ids = witcher.inv.AddAnItem(itemToCook, quantity);	
		
		Experience().AwardAlchemyBrewingXP(ids.Size(), witcher.inv.IsItemPotion(ids[0]), witcher.inv.IsItemOil(ids[0]), witcher.inv.IsItemBomb(ids[0]), isDistilling || witcher.inv.ItemHasTag(ids[0], 'MutagenIngredient'), witcher.inv.IsItemMutagenPotion(ids[0]));
		...
		if(ShouldProcessTutorial('TutorialAlchemyCook'))
		{
			...
			if( uiStateAlchemy )
			{
				uiStateAlchemy.CookedItem(recipe.recipeName);
			}
			else
			{
				...
				if( uiStateAlchemyMutagens )
					uiStateAlchemyMutagens.CookedItem(recipe.recipeName);
			}
		}
	}
	// W3EE - End
