...
class W3Effect_Burning extends W3CriticalDOTEffect
{
 	...
 	private function HandleAnimStopTime()
	{
		...
		baseDuration = 0.5f * PowF(2.0f, 2 - renewCounter);		
		
		target.RemoveTimer('StopBurningAnimation');
 		if (timeLeft > baseDuration)
			target.AddTimer('StopBurningAnimation', baseDuration + (timeLeft - baseDuration) * 0.4f);
	}
