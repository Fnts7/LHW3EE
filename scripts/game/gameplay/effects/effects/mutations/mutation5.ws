...
class W3Effect_Mutation5 extends CBaseGameplayEffect
{
	...
	{
		return RoundMath( 100 * bonusPerPoint * MinF(FloorF(target.GetStat( BCS_Focus )), 3.0f ) );
	}
	
	public function GetMaxStacks() : int
	{
		return RoundMath( 100 * bonusPerPoint * MinF( FloorF( target.GetStatMax( BCS_Focus ) ), 3.0f ) );
	}
}