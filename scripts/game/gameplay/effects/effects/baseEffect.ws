...
class CBaseGameplayEffect extends CObject
{
	...
	protected saved var isMutagenEffect : bool;
	
	default isMutagenEffect = false;
	// W3EE - End 						
	...
	// W3EE - Begin
	private var creator : CGameplayEntity;
	// W3EE - End
	...
	event OnEffectAdded(optional customParams : W3BuffCustomParams)
	{
		...
		if(isPotionEffect)
		{
			...
			if( thePlayer.GetStat(BCS_Toxicity) > thePlayer.GetStatMax(BCS_Toxicity) / 1.5 )
				customeffect.duration = 6.5f;
			else customeffect.duration = 1.3f;
	...
	// W3EE - Begin
	public function IsMutagenEffect() : bool
	{
		return isMutagenEffect;
	}
	// W3EE - End
	...
	{	
		...
		if( isActive && pauseCounters.Size() == 0)
		{
			...
			if( duration != -1 )
			{
				...
				/*
				{	
					if(isPotionEffect && !isMutagenEffect && isOnPlayer && thePlayer.CanUseSkill(S_Alchemy_s03) && effectType != EET_WhiteRaffardDecoction ) //modPrimer Delayed Recovery mutagen exclusion		
					{
						...
						if(thePlayer.GetStat(BCS_Toxicity, false) > toxicityThreshold)
						{
