...
class W3EffectManager
{
	...
	private final function InternalAddEffect(effectType : EEffectType, creat : CGameplayEntity, srcName : string, optional inDuration : float, optional customVal : SAbilityAttributeValue, optional customAbilityName : name, optional customFXName : name, optional signEffect : bool, optional powerStatValue : SAbilityAttributeValue, optional customParams : W3BuffCustomParams, optional vibratePadLowFreq : float, optional vibratePadHighFreq : float) : EEffectInteract
	{
		...
		if(effect)
		{
			...
			// W3EE - Begin
			if(signEffect && (effectType == EET_Confusion || effectType == EET_Hypnotized) && creat == thePlayer && GetWitcherPlayer().GetSignOwner().CanUseSkill(S_Magic_s17, GetWitcherPlayer().GetSignEntity(ST_Axii)) && GetWitcherPlayer().GetSignOwner().GetSkillLevel(S_Magic_s17, GetWitcherPlayer().GetSignEntity(ST_Axii)) == 3 && effect.GetDurationLeft() < CalculateAttributeValue(thePlayer.GetSkillAttributeValue(S_Magic_s17, 'duration_to_force_stagger', false, true)) )
			// W3EE - End
			{				
	...
	// W3EE - Begin
	public final function GetNumMutagensActive() : int
	{
		var i, ret : int;
		
		for(i=0; i<effects.Size(); i+=1)
		{
			if( effects[i].IsMutagenEffect() )
				ret += 1;
		}
	...
	public final function AddEffectsFromAction( action : W3DamageAction ) : bool
	{
		...
		{
			attackerPowerStatValue.valueMultiplicative *= 0.3f;
			attackerPowerStatValue.valueAdditive *= 0.3f;
		}
		...
		{		
			...
			else
			{
				...
				// W3EE - Begin
				resist = ((CNewNPC)owner).GetNPCCustomStat(theGame.params.DAMAGE_NAME_STUN);
				if( signEntity && signEntity.GetSignType() == ST_Aard && resist < 1.f )
				...
				resist = ((CNewNPC)owner).GetNPCCustomStat(theGame.params.DAMAGE_NAME_FIRE);
				if( signEntity && signEntity.GetSignType() == ST_Igni && !signEntity.IsAlternateCast() && resist < 1.f && Combat().ShouldIgniStagger(action) )
					ret = InternalAddEffect(EET_Stagger, action.attacker, action.GetBuffSourceName(), effectInfos[i].effectDuration, effectInfos[i].effectCustomValue, effectInfos[i].customFXName, effectInfos[i].effectAbilityName, signEntity, attackerPowerStatValue, effectInfos[i].effectCustomParam );
				// W3EE - End
			}
			...
			// W3EE - Begin
			if( effectInfos[i].effectType == EET_Bleeding && (ret == EI_Pass || ret == EI_Cumulate) )
