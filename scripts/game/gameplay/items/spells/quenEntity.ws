...
statemachine class W3QuenEntity extends W3SignEntity
{
	...
	protected function GetSignStats()
	{
		...
		// W3EE - Begin
		shieldDuration = CalculateAttributeValue(owner.GetSkillAttributeValue(S_Magic_4, 'shield_duration', true, true)) * 1.4f;
	...
	event OnStarted() 
	{
		...
		if( isAlternate || !owner.IsPlayer() )
		{
			...
			if( witcherOwner && owner.CanUseSkill( S_Magic_s14, this ) && witcherOwner.IsSetBonusActive( EISB_Bear_2 ) )
			{
...
state ShieldActive in W3QuenEntity extends Active
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		if( witcher && witcher.IsSetBonusActive( EISB_Bear_2 ) && parent.owner.CanUseSkill( S_Magic_s14, parent ) )
		{
	...
	public function ShowHitFX(damageData : W3DamageAction, rot : EulerAngles)
	{
		...
		{			
			...
			if(!isDoT)
			{
				...
				if( witcher && parent.owner.CanUseSkill( S_Magic_s14, parent ) && witcher.IsSetBonusActive( EISB_Bear_2 ) )
				{
