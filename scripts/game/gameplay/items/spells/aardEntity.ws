...
statemachine class W3AardEntity extends W3SignEntity
{
	...
	public final function GetDistance() : float
	{
		...
		{
			switch( owner.GetSkillLevel( S_Magic_s20, this ) )
			{
	...
	protected function ProcessThrow_MainTick( alternateCast : bool )
	{
		...
		{
			dispersionLevel = owner.GetSkillLevel(S_Magic_s20, this);
	...
	public final function PlayAardFX(hitsWater : bool)
	{
		...
		if(owner.CanUseSkill(S_Magic_s12, this))
		{
	...
	event OnThrowing()
	{
		...
		if( super.OnThrowing() )
		{
			...
			if( !parent.isFreeCast )
			{
				...
				if( ((W3Effect_SwordCritVigor)player.GetBuff(EET_SwordCritVigor)).GetReductionActive() )
				{
					((W3Effect_SwordCritVigor)player.GetBuff(EET_SwordCritVigor)).SetReductionActive(false);
					cost *= 0.6;
				}
			}
