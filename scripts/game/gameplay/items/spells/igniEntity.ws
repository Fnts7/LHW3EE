...
statemachine class W3IgniEntity extends W3SignEntity
{
	...
	protected function InitThrown()
	{
		...
		if(!IsAlternateCast())
		{
			...
			// W3EE - Begin
			if(owner.CanUseSkill(S_Magic_s09, this) || owner.GetActor().HasAbility('Glyphword 8 _Stats', true))
...
state IgniChanneled in W3IgniEntity extends Channeling
{
	...
	private function ProcessThrow(dt : float)
	{
		...
		// W3EE - Begin
		if(!casterActor.HasAbility('Glyphword 8 _Stats', true))
			aspectDist 		= parent.aspects[parent.fireMode].distance;
		else
			aspectDist 		= parent.aspects[parent.fireMode].distance + CalculateAttributeValue(casterActor.GetAttributeValue('glyphword8_range'));		
		// W3EE - End
