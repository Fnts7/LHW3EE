...
statemachine class W3YrdenEntity extends W3SignEntity
{
	...
	{
		// W3EE - Begin
		var chargesAtt, trapDurationAtt, sp : SAbilityAttributeValue;
		...
		charges = (int)(CalculateAttributeValue(chargesAtt) * sp.valueMultiplicative);
		trapDuration = CalculateAttributeValue(trapDurationAtt) * 1.75f * sp.valueMultiplicative;
		// W3EE - End
	}
	...
	private final function YrdenTrapHitEnemy(entity : CEntity, hitPosition : Vector)
	{
		...
		if ( casterActor && (CGameplayEntity)entity)
		{
			...
			if(player && skillLevel >= 1)
			{
				...
				damages.PushBack(SRawDamage(theGame.params.DAMAGE_NAME_ELEMENTAL, 210 * sp.valueMultiplicative * (1 + 0.25f * (skillLevel - 1)) ));
				...
				{
					damages[i].dmgVal += damageBonusFlat * skillLevel;
		...
		// W3EE - Begin
		if(casterActor.HasAbility('Glyphword 15 _Stats', true) && casterActor.GetStat(BCS_Focus) >= casterActor.GetStatMax(BCS_Focus))
		// W3EE - End
		{
