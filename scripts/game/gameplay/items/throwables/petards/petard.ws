	...
	{		
		...
		else
		{
			...
			if (!thePlayer.inv.GetItemModifierInt(itemId, 'ammo_current') )
			{
				...
				AddTimer('removeBombFromInventory',1.5f, 0);
			}	
			// W3EE - End
			...
			if( !ignoreBombSkills && (W3PlayerWitcher)GetOwner() && GetWitcherPlayer().CanUseSkill(S_Alchemy_s10) && !HasTag('Snowball'))
			{
				...
				for(j=0; j<atts.Size(); j+=1)
				{
					if(IsDamageTypeNameValid(atts[j]))
					{
						...
						if(index != -1)
						{
							params.damages[index].dmgVal += 100 * thePlayer.GetSkillLevel(S_Alchemy_s10); // CalculateAttributeValue(thePlayer.GetSkillAttributeValue(S_Alchemy_s10, atts[j], false, true));
						}
						else
						{
							newDamage.dmgType = atts[j];
							newDamage.dmgVal = 100 * thePlayer.GetSkillLevel(S_Alchemy_s10); // CalculateAttributeValue(thePlayer.GetSkillAttributeValue(S_Alchemy_s10, atts[j], false, true));
							params.damages.PushBack(newDamage);
						}
					}
				}
