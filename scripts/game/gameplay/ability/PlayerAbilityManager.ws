...
class W3PlayerAbilityManager extends W3AbilityManager
{
	...
	protected final function OnFocusChanged( optional isUndyingCall : bool )
	// W3EE - End
	{
		var points : float;
		...
		if( points >= 1.f && GetWitcherPlayer().IsMutationActive( EPMT_Mutation5 ) && !owner.HasBuff( EET_Mutation5 ) )
		// W3EE - End
		{
	...
	protected final function OnVitalityChanged()
	{
		...
		if(vitPerc < 1.f)
			ResetOverhealBonus();
		// W3EE - Begin
		else thePlayer.GetInjuryManager().ClearInjuries();
	...
	public final function IsSkillMutagenSlotUnlocked( eqSlot : EEquipmentSlots ) : bool
	{
		...
		// W3EE - Begin
		return Experience().GetTotalSkillPoints() >= mutagenSlots[ i ].unlockedAtLevel || Options().AllMutagensUnlocked();
		// W3EE - End
	}
