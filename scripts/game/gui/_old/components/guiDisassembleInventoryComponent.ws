...
class W3GuiDisassembleInventoryComponent extends W3GuiPlayerInventoryComponent
{
	...
	private function addRecyclingPartsList( item : SItemUniqueId, out flashObject : CScriptedFlashObject ) : void
	{
		...
		// W3EE - Begin
		itemQuantityPrice = _inv.GetItemPrice( item );
		itemQuantityPrice = Max(1, RoundMath( _inv.GetItemScaledPrice(itemQuantityPrice, item, _inv, true, false, true) * LFEGetDisassembleMult() * 0.2f ));
		// W3EE - End
