...
class W3TooltipComponent 
{
	...
	private function ApplyStatModifiers( wplayer : W3PlayerWitcher, inv : CInventoryComponent, item : SItemUniqueId, out primaryStat : float )
	{
		...
		switch(inv.GetItemCategory(item))
		{
			case 'armor' :
				primaryStat *=  MaxF(0, 1.f + ((primaryStat - 131) / 660));
				if( wplayer.HasAbility('Glyphword 11 _Stats', true) && wplayer.IsItemEquipped(item) )
				{
					switch(inv.GetArmorType(item))
					{
						case EAT_Light:		primaryStat += 225;	break;
						case EAT_Medium:	primaryStat += 225;	break;
						default : break;
					}
				}
			break;
			...
			case 'boots' :
				primaryStat *=  MaxF(0, 1.f + ((primaryStat - 40) / 171));
				if( wplayer.HasAbility('Glyphword 11 _Stats', true) && wplayer.IsItemEquipped(item) )
				{
					switch(inv.GetArmorType(item))
					{
						case EAT_Light:		primaryStat += 113;	break;
						case EAT_Medium:	primaryStat += 113;	break;
						default : break;
					}
				}
			break;
			...
			case 'gloves' :
				primaryStat *=  MaxF(0, 1.f + ((primaryStat - 38) / 166));
				if( wplayer.HasAbility('Glyphword 11 _Stats', true) && wplayer.IsItemEquipped(item) )
				{
					switch(inv.GetArmorType(item))
					{
	...
	public function GetBaseItemData( item : SItemUniqueId, itemInvComponent : CInventoryComponent, optional isShopItem : bool, optional compareWithItem : SItemUniqueId, optional compareItemInv : CInventoryComponent ) : CScriptedFlashObject
	{
			...
			else if (itemInvComponent.IsItemPotion(item))
			{
				...
				for (i = itemStats.Size() - 1; i >= 0; i -= 1)
				{
					if (itemStats[i].originName == 'duration')
						itemStats[i].value = wplayer.CalculatePotionDuration(item, definitionsMgr.ItemHasTag(itemName, 'Mutagen') );
				}
			}
			// W3EE - End
		...
		*/
		...
		// W3EE - Begin
		uniqueDescription = Equipment().GetRelicAbilityDescription(item, itemInvComponent);
		...
		if (!isArmorOrWeapon)
		{
			...
			else if (itemName == 'Mutagen 8')
				uniqueDescription = "Grants immunity to bleeding, burning and poison. Gives also small additional resistances to listed damage types.";
			else if (itemName == 'Mutagen 15')
				uniqueDescription = "Boosts your adrenaline gain. Your adrenaline gives also attack power bonus and poise increase - both values at rate: (current adrenaline percentage) / 4.";
			else if (itemName == 'Mutagen 20')
				uniqueDescription = "Reduces knockdown effect to stagger. Makes you much more resistant to any stagger (including breaking you from whirling). NOTE: Poise and whirl skill level still matter.";
			else if (StrContains(NameToString(itemName), "Blizzard 3"))
				uniqueDescription = "Slows down time when you kill an enemy or (shorter slowdown duration) counter with at least 1 vigor point. Slowdown times are 1 sec longer then in case of lower Blizzards. Also when you have max vigor on kill, slowdown is even 1 sec longer. Extended duration.";
			else
			...
			else if (StrContains(NameToString(itemName), "Blizzard 1") || StrContains(NameToString(itemName), "Blizzard 2"))
				uniqueDescription += "<br>Twice shorter slowdown will be also activated on counter if vigor is at least 2 points.";
		...
		*/
		...
		if ( m_shopInv )
		{
			...
			{
				// W3EE - Begin
				invItemPrice = m_shopInv.GetItemPrice(item);
				invItemPrice = m_shopInv.GetItemScaledPrice(invItemPrice, item, m_shopInv, true, false);
				// W3EE - End
				...
				if(GetFHUDConfig().showTrueItemPrice)
				{
					...
					invItemPrice = itemInvComponent.GetItemScaledPrice(invItemPrice, item, m_shopInv, true, false);
			...
			else
			{
				...
				// W3EE - Begin
				invItemPrice = m_playerInv.GetItemPrice( item );
				invItemPrice = m_playerInv.GetItemScaledPrice(invItemPrice, item, m_playerInv, true, true);
				// W3EE - End
				...
				else
				{
					...
					if(GetFHUDConfig().showTrueItemPrice)
					{
						weightValue = ((float)invItemPrice)*100.0f;
						invItemPrice = itemInvComponent.GetItemPrice(item);
						invItemPrice = itemInvComponent.GetItemScaledPrice(invItemPrice, item, m_playerInv, true, false);
						weightValue /= ((float)invItemPrice);
						invItemPriceString += " [" + invItemPrice + " : " + RoundMath(weightValue) + "%]";
					}
					//---=== modFriendlyHUD ===---
		...
		else
		{
			// W3EE - Begin
			invItemPrice = itemInvComponent.GetItemPrice(item);
			invItemPrice = itemInvComponent.GetItemScaledPrice(invItemPrice, item, itemInvComponent, true, false);
			// W3EE - End
			invItemPriceString = invItemPrice;
	...
	private function AddSetAttributes( itemId : SItemUniqueId, itemInvComponent : CInventoryComponent, out flashDataObj : CScriptedFlashObject ):bool
	{
		...
		if( setType != EIST_Undefined )
		{
			...
			{
				setAttribute = flashDataObj.CreateFlashObject();
				setAttribute.SetMemberFlashString( "value", Equipment().GetDescriptionColorByType(Equipment().GetMinimumItemsReqBySet(setType), setType, 1, currentCount, isActive1) );
			...
			if( desc2 != "" )
			{
				...
				setAttribute = flashDataObj.CreateFlashObject();
				setAttribute.SetMemberFlashString( "value", Equipment().GetDescriptionColorByType(theGame.params.ITEMS_REQUIRED_FOR_MAJOR_SET_BONUS, setType, 2, currentCount, isActive2) );
			...
			if (!ignorePrimaryStat || !currentStat.primaryStat)
			{
				...
				if (currentStat.originName == 'toxicity')
				{
					if (!isArmorOrWeapon)
					{
						...
						if (thePlayer.HasBuff(EET_Toxicity))
