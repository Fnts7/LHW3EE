...
class W3GuiPlayerInventoryComponent extends W3GuiBaseInventoryComponent
{
	...
	public function GetCraftedItemInfo(craftedItemName:name, targetObject:CScriptedFlashObject) : void
	{
		...
		if ( dm.IsItemSetItem( craftedItemName ) ) 
		{
			...
			{
				paramString.PushBack( (string)(Equipment().GetMinimumItemsReqBySet(setType)) );
				setBonusText += "<br><br>" + "<font color='#9f977a'>" + GetLocStringByKeyExtWithParams( "crafting_bonus_set_title",,, paramString ) + ":</font><br/>";
		...
		setBonusText = Equipment().GetRelicAbilityDescriptionByName(craftedItemName);
		if( setBonusText != "" )
		{
			targetObject.SetMemberFlashString("SetBonusDescription", setBonusText);
		...
		if( dm.ItemHasTag(craftedItemName, 'Wooden') )
			levelDifference = 40;
		else
		if( dm.ItemHasTag(craftedItemName, 'DamagedWeapon') )
			levelDifference = 55;
		else
		if( dm.ItemHasTag(craftedItemName, 'TypeBattleaxe') )
			levelDifference = 115;
		else
		if( dm.ItemHasTag(craftedItemName, 'TypeBattlemace') )
			levelDifference = 105;
		else
		if( dm.ItemHasTag(craftedItemName, 'SecondaryWeapon') )
			levelDifference = 78;
		
		if( craftedItemName == 'Aerondight EP2' )
			levelDifference = 95;
		
		switch( rarityId )
		{
			case 1: levelDifference -= 7; break;
			case 2: levelDifference -= 5; break;
			case 3: levelDifference -= 3; break;
		}
		levelDifference -= itemLevel;
		...
		if( levelDifference > 0 )
		{
			...
			{
				case 'armor' :
					primaryStatValue += 5 * levelDifference;
					primaryStatValue *=  MaxF(0, 1.f + ((primaryStatValue - 131) / 660));
				break;
				
				case 'boots' :
					primaryStatValue += 2 * levelDifference;
					primaryStatValue *=  MaxF(0, 1.f + ((primaryStatValue - 40) / 171));
				break;
				
				case 'gloves' :
					primaryStatValue += 2 * levelDifference;
					primaryStatValue *=  MaxF(0, 1.f + ((primaryStatValue - 38) / 166));
				break;
				...
				case 'silversword' :
					primaryStatValue += 10 * levelDifference;
					primaryStatValue = CeilF(primaryStatValue * 0.754f);
				break;
				
				case 'steelsword' :
					primaryStatValue += 8 * levelDifference;
				break;
		...
		if ( !dontCompare )
		{
			...
			{
				case 'armor' :
					eqPrimaryStatValue *=  MaxF(0, 1.f + ((eqPrimaryStatValue - 131) / 660));
					if( GetWitcherPlayer().HasAbility('Glyphword 11 _Stats', true) && GetWitcherPlayer().IsItemEquipped(equipedItemId) )
					{
						switch(playerInv.GetArmorType(equipedItemId))
						{
							case EAT_Light:		eqPrimaryStatValue += 225;	break;
							case EAT_Medium:	eqPrimaryStatValue += 225;	break;
							default : break;
						}
					}
				break;
				...
				case 'boots' :
					eqPrimaryStatValue *=  MaxF(0, 1.f + ((eqPrimaryStatValue - 40) / 171));
					if( GetWitcherPlayer().HasAbility('Glyphword 11 _Stats', true) && GetWitcherPlayer().IsItemEquipped(equipedItemId) )
					{
						switch(playerInv.GetArmorType(equipedItemId))
						{
							case EAT_Light:		eqPrimaryStatValue += 113;	break;
							case EAT_Medium:	eqPrimaryStatValue += 113;	break;
							default : break;
						}
					}
				break;
				...
				case 'gloves' :
					eqPrimaryStatValue *=  MaxF(0, 1.f + ((eqPrimaryStatValue - 38) / 166));
					if( GetWitcherPlayer().HasAbility('Glyphword 11 _Stats', true) && GetWitcherPlayer().IsItemEquipped(equipedItemId) )
					{
						switch(playerInv.GetArmorType(equipedItemId))
						{
	...
	public  function SetInventoryFlashObjectForItem( itemId : SItemUniqueId, out flashObject : CScriptedFlashObject) : void
	{
		...
		if (_shopInvCmp)
		{
			...
			// W3EE - Begin
			itemCost = GetInventoryComponent().GetItemPrice( itemId );
			itemCost = GetInventoryComponent().GetItemScaledPrice(itemCost, itemId, GetInventoryComponent(), true, true);
