...
{	
	...
	event  OnStartCrafting()
	{
		...
		// W3EE - Begin
		playerMoney = thePlayer.GetInventory().GetItemQuantityByName('Crowns');
		// W3EE - End
	}
	...
	event  OnCraftItem( tag : name )
	{
		...
		price = m_craftingManager.GetCraftingCost(tag);
		if( price > playerMoney )
		{
			diff = price - playerMoney;
	...
	private function PopulateData()
	{
		...
		{	
			...
			if(	m_craftingManager.GetSchematic(schematicName,schematic) )
			{
				...
				//W3EE - Ghetto Sorting ++
				dm.GetItemAttributeValueNoRandom(schematic.craftedItemName, false, 'quality', min, max );
				...
				if( l_GroupTag == 'armor' || l_GroupTag == 'gloves' || l_GroupTag == 'pants' || l_GroupTag == 'boots' || l_GroupTag == 'steelsword' || l_GroupTag == 'silversword' ) 
				{
					if( min.valueAdditive == 5 )
					{
						...
						if( l_GroupTag == 'boots' || l_GroupTag == 'gloves' || l_GroupTag == 'pants' )
							sortTier -= 1;
