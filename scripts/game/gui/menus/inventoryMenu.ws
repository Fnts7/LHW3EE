...
class CR4InventoryMenu extends CR4MenuBase
{
	...
	event  OnConfigUI()
	{
		...
		// W3EE - Begin
		else if ( theGame.GameplayFactsQuerySum("stashMode") == 1 || GetWitcherPlayer().GetInputHandler().GetWasStashCall() )
		{
			GetWitcherPlayer().GetInputHandler().SetWasStashCall();
	...
	event  OnClosingMenu()
	{
		...
		{
			Equipment().ToggleHairItem(false, GetWitcherPlayer(), true);
		}
		else Equipment().ToggleHairItem(true, GetWitcherPlayer(), true);
		// W3EE - End
	...
	event  OnPutInSocket( item : SItemUniqueId )
	{
		...
		else if( !GetWitcherPlayer().IsMeditating() && !Options().GetUpgradeAnywhere() )
		{
	...
	event  OnSellItem( itemId : SItemUniqueId, quantity : int )
	{
		...
		// W3EE - Begin
		itemPrice = _playerInv.GetInventoryComponent().GetItemPrice( itemId );
		itemPrice = _playerInv.GetInventoryComponent().GetItemScaledPrice(itemPrice, itemId, _playerInv.GetInventoryComponent(), true, true);
	...
	event  OnBuyItem( item : SItemUniqueId, quantity : int, moveToIdx : int )
	{
		...
		{
			// W3EE - Begin
			price = _shopInv.GetInventoryComponent().GetItemPrice(item);
			price = _shopInv.GetInventoryComponent().GetItemScaledPrice(price, item, _shopInv.GetInventoryComponent(), true, false);
	...
	private function OpenQuantityPopup( itemId : SItemUniqueId, action : EQuantityTransferFunction, maxValue : int )
	{
		...
		switch( action )
		{
			case QTF_Sell:
				invItem = _playerInv.GetInventoryComponent().GetItem( itemId );
				// W3EE - Begin
				_quantityPopupData.itemCost = _playerInv.GetInventoryComponent().GetItemPrice( itemId );
				_quantityPopupData.itemCost = _playerInv.GetInventoryComponent().GetItemScaledPrice(_quantityPopupData.itemCost, itemId, _playerInv.GetInventoryComponent(), true, true);
				// W3EE - End
			...
			case QTF_Buy:
				// W3EE - Begin
				_quantityPopupData.itemCost = _shopInv.GetInventoryComponent().GetItemPrice( itemId );
				_quantityPopupData.itemCost = _shopInv.GetInventoryComponent().GetItemScaledPrice(_quantityPopupData.itemCost, itemId, _shopInv.GetInventoryComponent(), true, false);
				// W3EE - End
				_quantityPopupData.showPrice = true;
	...
	function SellItem( item : SItemUniqueId, quantity : int ) : SItemUniqueId
	{
		...
		if( _shopNpc )
		{
			...
			// W3EE - Begin
			itemCost = _playerInv.GetInventoryComponent().GetItemPrice( item ) * quantity;
			itemCost = _playerInv.GetInventoryComponent().GetItemScaledPrice(itemCost, item, _playerInv.GetInventoryComponent(), true, true);
			// W3EE - End
	...
	protected function createInventoryStashContext():void
	{
		...
		_invStashContext.SetInventoryRef(this);
	...
	function GetCurrentWeight( inv : CInventoryComponent ) : float
	{
		...
		for(i=0; i<items.Size(); i+=1)
			if( !_horseInv.isHorseItem(items[i]) && inv.IsItemEncumbranceItem(items[i]) )
	...
	public function GetItemEncumbranceWithQuantity( invComponent : CInventoryComponent, item : SItemUniqueId, quantity : int ) : float
	{
		...
		if ( invComponent.IsItemEncumbranceItem( item ) )
		{
			weight = invComponent.GetItemWeight( item );
			
			if ( invComponent.GetItemName( item ) == 'Crowns' || invComponent.GetItemName( item ) == 'Florens' || invComponent.GetItemName( item ) == 'Orens' )
			{
			...
			else if ( invComponent.IsItemQuest( item ) )
			{
				if( Options().WeightQuest() )
				{
					if ( _inv.GetItemName( item ) == 'Geralt robbery mask' )
					{
						return 0;
					}
					else if( weight > 0.05 )
					{
						return weight * _inv.GetItemQuantity( item );
					}
					else
					{
						return 0.05 * _inv.GetItemQuantity( item );
					}
				}
				else
					return 0;
			}	
			else if ( invComponent.IsItemCraftingIngredient( item ) )
			{
				if( weight > 0.05 )
				{
				...
				{
					return 0.05 * quantity;
				}
			}
			else if ( invComponent.IsItemAlchemyIngredient( item ) )
			{
			...
			else if ( invComponent.IsItemSingletonItem( item ) && invComponent.IsItemAlchemyItem( item ) )
			{
				if ( invComponent.IsItemOil( item ) || invComponent.IsItemPotion( item ) )
				{
					return 0.25 * quantity * invComponent.GetItemModifierInt( item, 'ammo_current' );
				}
				else if ( invComponent.IsItemBomb( item ) )
				{
					return 0.5 * quantity * invComponent.GetItemModifierInt( item, 'ammo_current' );
				}
			}
			else 
			{
		...
		for(i=0; i<items.Size(); i+=1)
		{
			if( _inv.IsItemEncumbranceItem(items[i]) )
			{
				...
				if( _inv.IsItemAlchemyIngredient(items[i]) )
				{
					...
					if( !mut )
						handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i]));
				}
				else
				if( _inv.GetItemName(items[i]) == 'Crowns' )
				{
					if( _inv.GetItemQuantity(items[i]) > 100 )
						handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i]) - 100);
				}
				else
				if( _inv.IsItemCraftingIngredient(items[i]) || _inv.GetItemName(items[i]) == 'Florens' || _inv.GetItemName(items[i]) == 'Orens' )
				{
					handleMoveToStashQuantity(items[i], _inv.GetItemQuantity(items[i]));
				}
				else
	...
	public function MoveToStash(item : SItemUniqueId) : bool
	{
		...
		else
		{
			...
			{
				showNotification("Saddlebag is full.");
				OnPlaySoundEvent("gui_global_denied");
				return false;
			}
            ...
            InventoryRemoveItem(item, false);
            _playerInv.clearGridPosition(item);
            witcher.GetHorseManager().MoveItemToHorse(item, 1);
            UpdateHorseInventory();
            UpdateEncumbranceInfo();
		}
	...
	public function handleMoveToStashQuantity(item : SItemUniqueId, quantity : int) : bool
	{
		...
		if( newWeight > maxWeight )
		{
 		...
 		witcher.GetHorseManager().MoveItemToHorse(item, quantity); 
		if( quantity >= _inv.GetItemQuantity(item) )
		{
			InventoryRemoveItem(item, false);
			_playerInv.clearGridPosition(item);
		}
