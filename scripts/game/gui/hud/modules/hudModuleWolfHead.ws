...
class CR4HudModuleWolfHead extends CR4HudModuleBase
{
	...
	event OnTick( timeDelta : float )
	{
		...
		// W3EE - Begin
		if ( isFadingOut || IsVisibleTemporarily() || thePlayer.IsCombatMusicEnabled() /*|| (m_curToxicity > 0.f || m_lockedToxicity > 0.f)*/ || (m_curVitality < m_maxVitality) || thePlayer.GetStat(BCS_Focus) < thePlayer.GetStatMax(BCS_Focus) || thePlayer.GetStat(BCS_Stamina) < thePlayer.GetStatMax(BCS_Stamina) )
		// W3EE - End
			SetAlwaysDisplayed( true );
		else
