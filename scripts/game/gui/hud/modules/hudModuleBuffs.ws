	...
	function UpdateBuffs()
	{
		...
		for(i = 0; i < Min(buffDisplayLimit,_currentEffects.Size()); i += 1) 
		{
			if(_currentEffects[i].ShowOnHUD() && _currentEffects[i].GetEffectNameLocalisationKey() != "MISSING_LOCALISATION_KEY_NAME" )
			{
				...
				else*/ if ( /*effectType == EET_Oil ||*/ effectType == EET_Aerondight || effectType == EET_BasicQuen || effectType == EET_WinterBlade || effectType == EET_PhantomWeapon || effectType == EET_DimeritiumCharge )
				// W3EE - End
				{
					
					format = 1;
				}
				else if ( effectType == EET_Mutation3 || effectType == EET_Mutation4 || effectType == EET_Mutation5 || effectType == EET_Mutation10 || effectType == EET_Oil || effectType == EET_CombatAdr )
				{
				...
				{	
					...
					if ( oilEffect )
					{
						...
						l_flashObject.SetMemberFlashNumber("initialDuration", CeilF(oilEffect.GetAmmoPercentage() * 100.f));
						// W3EE - End
					}
				}
				else if( effectType == EET_Aerondight )
