...
class CModMarkers
{
	...
	var cached3DMarkers						: array< SMod3DMarker >;
	...
	function UpdateCompassScreenPosition( ID : int )
	{
		...
		if( text != "" )
		{
			...
			if( isCompassFadingOut )
				mcOneliner.SetAlpha( 100 * MaxF( 0, compassFadeOutTime / config.markersFadeOutDelay ) );
			else
				mcOneliner.SetAlpha( 100 );
		}
		else
		{
			if( mcOneliner )
	...
	function Update3DMarkers()
	{
		...
		var i						: int;
		var marker, userMarker		: SMod3DMarker;
		var hasUserMarker, userMarkerSubstituted, showUser, isUser : bool;
		
		if( isDirty && delayedCacheTime - theGame.GetEngineTimeAsSeconds() <= 0 )
		{
			Cache3DMarkers();
		}
		else
		...
		for( i = 0; i < markersCount; i += 1 )
		{
			...
			marker.distance = VecDistance( playerPosition, marker.position );

			if( marker.distance < config.markerMinDistance )
				continue;
			
			isUser = IsUserMain( marker );
			
			//user pinned markers ignore both visibility and max distance settings
			if( !isUser && hasUserMarker && !userMarkerSubstituted && VecDistance2D( marker.position, userMarker.position ) <= 4.f )
			{
			...
			if( !marker.pin && marker.distance > config.markerMaxDistance )
				continue;
		...
		if( showUser && !userMarkerSubstituted )
			UpdateMarker( userMarker );
	...
	function UpdateMarker( out marker : SMod3DMarker )
	{
		...
		{
			markerDescrText = "<font" + GetDescriptionFont() + ">" + marker.description + "</font>";
		...
		if( marker.onelinerID )
		{
			...
			if( marker.onScreen || !noOppositeCamera )
			{
				...
				if( isIconFadingOut && !marker.force )
					mcOneliner.SetAlpha( 100 * MaxF( 0, iconFadeOutTime / config.markersFadeOutDelay ) );
				else
					mcOneliner.SetAlpha( 100 );
			}
			else
		...
		if( marker.descrOnelinerID )
		{
			...
			if( marker.onScreen )
			{
				...
				if( isDescriptionFadingOut )
					mcOneliner.SetAlpha( 100 * MaxF( 0, descriptionFadeOutTime / config.markersFadeOutDelay ) );
				else
					mcOneliner.SetAlpha( 100 );
			}
			else
		...
		if( marker.distOnelinerID )
		{
			...
			if( marker.onScreen )
			{
				...
				if( isDistanceFadingOut )
					mcOneliner.SetAlpha( 100 * MaxF( 0, distanceFadeOutTime / config.markersFadeOutDelay ) );
				else
					mcOneliner.SetAlpha( 100 );
			}
			else
	...
	{
		return "<font" + GetDistanceFont() + ">" + FloatToStringPrec( FloorF( marker.distance ), 0 ) + "</font>";
	}
	...
	function Cache3DMarkers()
	{
		...
		for( i = 0; i < mapPinInstancesCount; i += 1 )
		{
			...
			if( IsDynamic( marker ) )
				mDynamic.PushBack( marker );
			else
				mStatic.PushBack( marker );
